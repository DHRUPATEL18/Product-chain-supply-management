...
if ($_SERVER["REQUEST_METHOD"] === "GET" && isset($_GET['dist'])) {
    $dist_name = $_GET['dist'] ?? '';
    $s_date = $_GET['s_date'] ?? '';
    $e_date = $_GET['e_date'] ?? '';
    ...
    if (!empty($s_date) && !empty($e_date)) {
        ...
        WHERE bd.assigned_to = '$dist_name'
    ...
    if ($restable && mysqli_num_rows($restable) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($restable)) {
            ...
            echo "<td><button class='btna btn-view' onclick='dbloadr(" . $row['batch_id'] . ")'><i class='fas fa-eye'></i></button></td>";
            echo "</tr>";
        }
