...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $qr = "UPDATE batch_distributor SET assigned_by = ?, assigned_to = ?, assigned_at = NOW(), status	 = ? WHERE id = ?";
    $stmt = mysqli_prepare($cn, $qr);
    mysqli_stmt_bind_param($stmt, "iisi", $assigned_by, $assigned_to, $status, $id);


    if (mysqli_stmt_execute($stmt)) {
        setcookie("edit_id", "", time() - 3600);
        header("Location: ../tablegrid.php?tn=batch_distributor");
        exit();
    } else {
